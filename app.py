...
def obtener_financial_insights_yf(ticker):
    """Scrapea el análisis financiero de Yahoo Finance"""
    try:
        ...
        # Buscar el contenido del análisis financiero
...
def obtener_financial_insights_peers(peers_tickers):
    """Obtiene los financial insights de todos los peers"""
    insights_dict = {}
    for ticker in peers_tickers:
        insight = obtener_financial_insights_yf(ticker)
        if insight:
            insights_dict[ticker] = insight
...
def obtener_income_yahoo(ticker):
    try:
        ...
        data = []
        for m in df.columns:
            vals = [f"{v/1e9:,.1f} B" if abs(v) >= 1e9 else f"{v/1e6:,.0f} M" if abs(v) >= 1e6 else f"{v:,.0f}" for v in df[m]]
            data.append([m] + vals)
...
def calcular_rendimientos_riesgos(ticker, indice_ticker, periodos_config, periodo_historico):
    ...
    for p in periodos_config:
        try:
            ...
            if len(pp) < 10:
                resultados.append({"Periodo": p['nombre'], "Rendimiento (%)": "N/A", "Volatilidad (%)": "N/A", "Beta": "N/A", "Alpha (%)": "N/A", "Sharpe Ratio": "N/A"})
                continue
            
            rend = ((pp.iloc[-1] / pp.iloc[0]) - 1) * 100
            vol = rp.std() * np.sqrt(252) * 100
            beta = np.cov(rp, rip)[0, 1] / np.var(rip) if len(rp) > 1 and len(rip) > 1 and np.var(rip) != 0 else 0
            rend_ind = ((precios_indice[precios_indice.index.isin(pp.index)].iloc[-1] / precios_indice[precios_indice.index.isin(pp.index)].iloc[0]) - 1) * 100
            alpha = rend - (beta * rend_ind)
            sharpe = (rp.mean() * 252 / rp.std() / np.sqrt(252)) if rp.std() != 0 else 0
            ...
            resultados.append({"Periodo": p['nombre'], "Rendimiento (%)": "Error", "Volatilidad (%)": "Error", "Beta": "Error", "Alpha (%)": "Error", "Sharpe Ratio": "Error"})
...
with st.sidebar:
    ...
    st.divider()
    st.markdown("**v3.2** | Ingeniería Financiera")
    ...
    with st.spinner("Analizando..."):
        try:
            ...
            if not datos.empty:
                ...
                fig = go.Figure(go.Candlestick(x=datos.index, open=get_col(datos, 'Open'), high=get_col(datos, 'High'),
                                               low=get_col(datos, 'Low'), close=get_col(datos, 'Close'),
                                               increasing_line_color='#26A65B', decreasing_line_color='#C0392B'))
            ...
            indice_ticker = indices_dict[indice_select]
            df_analisis = calcular_rendimientos_riesgos(ticker, indice_ticker, periodos_personalizados, periodo_historico.lower())
            ...
            if not df_graf.empty:
                ...
                fig_rend = go.Figure(go.Bar(x=df_graf["Periodo"], y=df_graf["Rend_num"],
                                            marker_color=['#26A65B' if x > 0 else '#C0392B' for x in df_graf["Rend_num"]],
                                            text=df_graf["Rend_num"].round(2), textposition='outside'))
                fig_rend.update_layout(title="Rendimientos por Periodo", template="plotly_white", height=400, showlegend=False)
            ...
            if peers and not df_comp.empty:
                ...
                with st.spinner("Obteniendo insights de peers y generando análisis..."):
                    # Obtener financial insights de peers
                    ...
                    modelo = genai.GenerativeModel("gemini-2.5-flash")
                    prompt_peers = f"""Eres un analista financiero senior especializado en análisis comparativo de empresas.
                    ...
                    1. POSICIÓN COMPETITIVA (2-3 oraciones):
                       - ¿Cómo se posiciona {ticker} frente a sus peers en términos de valoración (P/E), rentabilidad (ROE, márgenes) y tamaño (Market Cap)?
                    
                    2. FORTALEZAS RELATIVAS (3 puntos concretos):
                       - Identifica 3 ventajas específicas que {ticker} tiene sobre sus competidores
                    
                    3. DEBILIDADES RELATIVAS (3 puntos concretos):
                       - Identifica 3 áreas donde {ticker} está por debajo de sus peers
                    
                    4. OPORTUNIDADES Y RIESGOS (2-3 oraciones):
                       - Basándote en los financial insights, ¿qué tendencias u oportunidades destacan?
                       - ¿Qué riesgos específicos enfrenta {ticker} vs sus competidores?
                    
                    5. RECOMENDACIÓN DE INVERSIÓN (clara y directa):
                       - COMPRAR / VENDER
                       - Justificación en 2-3 oraciones basadas en la comparación
                       - ¿Qué peer alternativo recomiendas si no es {ticker}?
                    
                    Usa lenguaje profesional, datos específicos de la tabla, y sé contundente en tu recomendación.
                    Formatea con bullets y negritas para mejor legibilidad. Da toda la respuesta en un texto plano, sin formatos markdowns ni astericos para representar formatos bold."""
                ...
                st.markdown(
                    ...
                    <div style='background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); 
                        ...
                        <p style='margin-bottom: 0; margin-top: 15px; font-size: 11px; opacity: 0.9;'>
                            🤖 Análisis generado por Gemini AI comparando {ticker} con {len(peers[:5])} competidores
                        </p>
                    </div>
                    """,
                    unsafe_allow_html=True
            ...
            modelo = genai.GenerativeModel("gemini-2.5-flash")
            prompt = f"""Eres un analista financiero profesional. Analiza {ticker} con los siguientes datos:
            
            MÉTRICAS CLAVE:
            Market Cap: {info.get('marketCap')}, P/E: {info.get('trailingPE')}, ROE: {info.get('returnOnEquity')}, 
            Beta: {info.get('beta')}, EPS: {info.get('trailingEps')}, Gross Margin: {info.get('grossMargins')},
            Profit Margin: {info.get('profitMargins')}, Debt/Equity: {info.get('debtToEquity')}
            Sector: {info.get('sector')}, Industria: {info.get('industry')}
            ...
            Proporciona un análisis en MÁXIMO 280 palabras con:
            
            1. RECOMENDACIÓN (COMPRAR / VENDER) - Justifica en 2 oraciones 
            2. FORTALEZAS (3 puntos clave con datos específicos)
            3. RIESGOS (3 puntos con datos específicos)
            4. VALORACIÓN (¿Sobrevalorada / Bien valorada / Subvalorada? - 2 oraciones)
            5. PERSPECTIVA CORTO PLAZO (3-6 meses - 2 oraciones)
            6. PERSPECTIVA LARGO PLAZO (1-3 años - 2 oraciones)
            
            Usa lenguaje profesional, datos concretos, y estructura con bullets.Da toda la respuesta en un texto plano, sin formatos markdowns ni astericos para representar formatos bold."""
            ...
            st.warning("⚠️ **Advertencia Legal:** Este análisis es generado por IA con fines exclusivamente informativos y educativos. NO constituye asesoramiento financiero, recomendación de inversión, ni oferta de compra/venta de valores. Consulte con un asesor financiero certificado antes de tomar decisiones de inversión. Las inversiones conllevan riesgos y pueden resultar en pérdidas de capital.")
...
<div style='text-align:center; color:gray; font-size:11px;'>
    ...
    🎓 <b>Proyecto:</b> Ingeniería Financiera | 💻 <b>Versión:</b> 3.2 | ⚖️ <b>Disclaimer:</b> Solo fines educativos
</div>
""", unsafe_allow_html=True)